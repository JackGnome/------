...
import store.jackgnome.djarenaservice.model.product.ProductSearchFilterDto
...
class ProductSearchService : ApplicationListener<ApplicationReadyEvent> {
    ...
    fun getFilters(): ProductSearchFilterDto {
        val brands = brandService.getAll(isArchived = false).map { it.name }.toSet()
        val minPrice = productRepository.findMinProductPrice()
        val maxPrice = productRepository.findMaxProductPrice()

        return ProductSearchFilterDto(brands = brands, minPrice = minPrice, maxPrice = maxPrice)
    }
