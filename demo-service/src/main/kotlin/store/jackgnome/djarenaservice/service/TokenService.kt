...
class TokenService {
    ...
    @Transactional
    fun revokeTokensByAccessTokenValue(value: String) {
        val accessToken = authTokenRepository.findByValue(value) ?: throw InvalidAuthTokenException(value)
        revokeTokens(accessToken.user, accessToken.deviceId)
    }

    @Scheduled(cron = "0 5 * * * * ") // 5 minutes
    @Transactional
    fun clearTokens() {
        authTokenRepository.deleteAllByRevoked(true)
    }
